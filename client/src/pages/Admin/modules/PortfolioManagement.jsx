      ...
      <AnimatePresence>
        {modalType === 'category' && (
          <div className="fixed inset-0 z-[210] flex items-center justify-center p-4">
            ...
            <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white w-full max-w-md rounded-sm shadow-2xl relative z-10 overflow-hidden">
              ...
              <div className="p-6 space-y-8">
                ...
                <form onSubmit={async (e) => { e.preventDefault(); if(!newCategoryName) return; setIsSubmitting(true); try { await api.post('/portfolio/categories', { name: newCategoryName }); setNewCategoryName(''); fetchData(); } catch(err){ alert('Fail'); } finally { setIsSubmitting(false); } }} className="space-y-6 pt-6 border-t border-gray-100">
        ...
        {(modalType === 'album' || modalType === 'editAlbum') && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10">
            <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModalType(null)} className="absolute inset-0 bg-dark/80 backdrop-blur-md" />
            <m.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="bg-white w-full max-w-2xl max-h-[90vh] rounded-sm shadow-2xl relative z-10 overflow-hidden flex flex-col">
