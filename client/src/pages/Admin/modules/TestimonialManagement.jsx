  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div><h3 className="text-3xl font-serif text-main mb-2 italic">Client Testimonials</h3><p className="text-[10px] uppercase tracking-widest text-gray-400 font-bold">Manage customer feedback</p></div>
        <button onClick={() => { 
          setModalType('add'); 
      ...
      <AnimatePresence>
        {(modalType === 'add' || modalType === 'edit') && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10">
            <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setModalType(null)} className="absolute inset-0 bg-dark/80 backdrop-blur-md" /><m.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="bg-surface w-full max-w-2xl max-h-[90vh] rounded-sm shadow-2xl relative z-10 overflow-hidden flex flex-col"><div className="p-8 border-b border-surface flex justify-between items-center bg-secondary/30 shrink-0"><h4 className="text-2xl font-serif text-main italic">{modalType === 'add' ? 'New Testimonial' : 'Edit Review'}</h4><button onClick={() => setModalType(null)} className="text-gray-400 hover:text-main" aria-label="Close"><X size={28} /></button></div><form onSubmit={handleSubmit} className="p-10 space-y-8 overflow-y-auto custom-scrollbar"><div className="space-y-2"><label className="text-[10px] uppercase tracking-widest font-bold text-primary">Client Name</label><input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full p-4 bg-secondary border-b border-transparent focus:border-primary outline-none font-bold text-main font-sans" /></div><div className="space-y-2"><label className="text-[10px] uppercase tracking-widest font-bold text-primary">Rating</label><div className="flex gap-4 p-4 bg-secondary/50 rounded-sm">{[1,2,3,4,5].map(num => <button key={num} type="button" onClick={() => setFormData({...formData, rating: num})} className={`transition-all ${formData.rating >= num ? 'text-yellow-400 scale-125' : 'text-gray-300'}`}><Star size={24} className={formData.rating >= num ? "fill-yellow-400" : ""} /></button>)}</div></div><div className="space-y-2"><label className="text-[10px] uppercase tracking-widest font-bold text-primary">Comment</label><textarea rows="4" required value={formData.comment} onChange={e => setFormData({...formData, comment: e.target.value})} className="w-full p-4 bg-secondary border-b border-transparent focus:border-primary outline-none resize-none font-serif italic text-main" /></div><div className="space-y-2 pb-6"><label className="text-[10px] uppercase tracking-widest font-bold text-primary">Photo / Logo</label><div onClick={() => fileInputRef.current.click()} className="border-2 border-dashed border-gray-200 hover:border-primary p-12 text-center cursor-pointer bg-dark group rounded-sm flex flex-col items-center relative overflow-hidden"><div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]" /><input type="file" ref={fileInputRef} onChange={(e) => { const f=e.target.files[0]; if(f){ setImageFile(f); setImagePreview(URL.createObjectURL(f)); } }} className="hidden" accept="image/*" />{imagePreview ? <img src={imagePreview} className="w-32 h-32 rounded-full object-contain border-4 border-white/20 shadow-xl relative z-10" /> : <User size={40} className="text-gray-500 relative z-10" />}<p className="mt-4 text-[9px] uppercase tracking-widest text-gray-400 font-bold relative z-10">Supports Transparent PNGs</p></div></div></form><div className="p-8 bg-surface-hover border-t border-surface flex gap-4 shrink-0"><button type="button" onClick={() => setModalType(null)} className="flex-1 py-4 bg-surface border border-gray-200 text-main uppercase tracking-widest text-[10px] font-bold">Cancel</button><button onClick={handleSubmit} disabled={isSubmitting} className="flex-[2] py-4 bg-dark text-white uppercase tracking-widest text-[10px] font-bold hover:bg-primary transition-all rounded-sm flex items-center justify-center gap-3">{isSubmitting ? <Loader2 size={16} className="animate-spin" /> : 'Publish'}</button></div></m.div>
          </div>
        )}
      </AnimatePresence>
