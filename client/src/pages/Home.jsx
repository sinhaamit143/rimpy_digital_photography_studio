  ...
  const fetchData = async () => {
    try {
      ...
      if (googleRes.status === 'fulfilled' && googleRes.value.data?.success && googleRes.value.data.data?.length > 0) {
        ...
        // Capture aggregate stats (rating + total count) for the trust badge
        if (googleRes.value.data.stats) {
          setGoogleStats(googleRes.value.data.stats);
        }
      } 
        ...
        <section className="py-32 bg-dark text-white relative overflow-hidden section-testimonials">
          <div className="container relative z-10">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <h2 className="text-3xl md:text-5xl mb-6 md:mb-8 italic font-serif leading-tight">Kind Words from <br className="md:hidden" /> our Clients</h2>

              {/* ── Live Google Trust Badge ── */}
              {googleStats?.totalRatings && (
                <div className="inline-flex flex-wrap items-center justify-center gap-3 md:gap-5 mb-10 md:mb-14 px-6 py-3 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm">
                  {/* Stars */}
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        size={16}
                        className={i < Math.round(googleStats.averageRating || 5) ? 'text-accent fill-accent' : 'text-white/20'}
                      />
                    ))}
                  </div>
                  {/* Count */}
                  <span className="text-white font-bold text-sm md:text-base tracking-wide">
                    {googleStats.totalRatings >= 1000
                      ? `${(googleStats.totalRatings / 1000).toFixed(1)}k+`
                      : `${googleStats.totalRatings}+`}{' '}
                    <span className="font-normal text-white/70">Five-Star Ratings</span>
                  </span>
                  {/* Divider */}
                  <span className="hidden md:block w-px h-4 bg-white/20" />
                  {/* Google Badge */}
                  <div className="flex items-center gap-2 text-white/60 text-xs font-medium tracking-widest uppercase">
